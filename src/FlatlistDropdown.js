...
export default class FlatlistDropdown extends Component {
  static propTypes = {
  ...
  _renderModal() {
    const { animationType, accessible, dropdownStyle } = this.props;
  ...
  _renderDropdown() {
    const { renderSeparator, showsVerticalScrollIndicator, options } = this.props;
    return (
      <FlatList
        style={styles.list}
        data={options}
  ...
  _onRowPress(rowData, rowID) {
    const { onSelect, renderButtonText, onDropdownWillHide, onValueChange } = this.props;
    if (!onSelect || onSelect(rowID, rowData) !== false) {
      const value = renderButtonText && renderButtonText(rowData) || rowData.value.toString();
      this._nextValue = value;
      ...
      onValueChange && onValueChange(value, rowID)
    }
